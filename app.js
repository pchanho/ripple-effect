...
class App {
    constructor() {
        ...
        window.devicePixelRatio -> returns the ratio of the resolution
                                   to the resolution in CSS pixels for the current display device
         1 - class display, 2 - Retina display **/
        ...
        this.image.src = "the-splash.jpeg";
        this.image.onload = () => {
            this.isLoaded = true;
            this.drawImage();
        };
    ...
    drawDots() {
        ...
        // Math.ceil = round number up to the next largest integer.

        for (let i = 0; i < this.rows; i++){
            let y = (i + 0.5) * this.pixelSize;
            let pixelY = Math.max(Math.min(y, this.stageHeight), 0);

            for (let j = 0; j < this.columns; j++){
                let x = (j + 0.5) * this.pixelSize;
                let pixelX = Math.max(Math.min(x, this.stageWidth), 0);

                let pixelIndex = (pixelX + pixelY * this.stageWidth) * 4;
                let red = this.imgData.data[pixelIndex];
                let green = this.imgData.data[pixelIndex + 1];
                let blue = this.imgData.data[pixelIndex + 2];
